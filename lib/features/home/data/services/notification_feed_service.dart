...
class NotificationFeedService {
  static Future<List<AppNotification>> load({SoccerData? soccerData}) async {
    ...
    try {
      results.addAll(await NewsService.fetchFootballNews());
    } catch (_) {}

    if (soccerData != null) {
      for (final match in soccerData.matches.where((m) => m.isLive)) {
        final score = match.score.map((value) => value.toString()).join(' - ');
        results.add(AppNotification(
          id: 'match_${match.id}',
          title: '${match.homeTeam} vs ${match.awayTeam}',
          body: score.isEmpty ? 'Partido en vivo ahora' : 'En vivo · $score',
          source: 'Pivote Fútbol',
