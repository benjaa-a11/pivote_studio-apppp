...
class SupportScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: const PivoteAppBar(
        ...
        subtitle: 'Resolvé dudas y encontrá ayuda rápida',
      ),
      body: ListView(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(child: _social(context, FontAwesomeIcons.instagram, 'Instagram', const Color(0xFFE4405F))),
              const SizedBox(width: 9),
              Expanded(child: _social(context, FontAwesomeIcons.xTwitter, 'X', theme.colorScheme.onSurface)),
              const SizedBox(width: 9),
              Expanded(child: _social(context, FontAwesomeIcons.facebookF, 'Facebook', const Color(0xFF1877F2))),
            ],
          ),
          ...
          Row(
            children: [
              ...
              Expanded(child: Text('Para soporte técnico, incluí el modelo de tu dispositivo y una descripción breve del problema.', style: GoogleFonts.spaceGrotesk(fontSize: 10.5, fontWeight: FontWeight.w600, color: theme.hintColor, height: 1.4))),
            ],
          ),
        ],
      ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(width: 48, height: 48, decoration: BoxDecoration(color: Colors.black.withValues(alpha: .09), borderRadius: BorderRadius.circular(15)), child: const Icon(Icons.support_agent_rounded, color: Colors.white, size: 25)),
              const Spacer(),
              Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 6), decoration: BoxDecoration(color: Colors.white.withValues(alpha: .14), borderRadius: BorderRadius.circular(999)), child: Text('PIVOTE CARE', style: GoogleFonts.spaceGrotesk(fontSize: 9, fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: 1))),
            ],
          ),
          const SizedBox(height: 20),
          Text('Estamos para ayudarte', style: GoogleFonts.spaceGrotesk(color: Colors.white, fontSize: 23, fontWeight: FontWeight.w900)),
          ...
          Row(
            children: [
              Expanded(child: _contact(context, Icons.email_outlined, 'Email', 'mailto:soporte@pivote.com')),
              const SizedBox(width: 9),
              Expanded(child: _contact(context, FontAwesomeIcons.telegram, 'Telegram', 'https://t.me/pivote_support')),
            ],
          ),
        ],
      ),
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              if (icon is IconData) Icon(icon, color: Colors.white, size: 17) else FaIcon(icon as FaIconData, color: Colors.white, size: 17),
              const SizedBox(width: 7),
              Text(label, style: GoogleFonts.spaceGrotesk(color: Colors.white, fontSize: 11.5, fontWeight: FontWeight.w800)),
            ],
          ),
        ),
      ),
  ...
  Widget _social(BuildContext context, FaIconData icon, String label, Color color) {
    final theme = Theme.of(context);
    return Material(
      color: theme.colorScheme.surface,
      ...
      child: InkWell(
        ...
        child: Container(
          padding: const EdgeInsets.symmetric(vertical: 12),
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), border: Border.all(color: color.withValues(alpha: .12))),
          child: Column(
            children: [
              FaIcon(icon, size: 17, color: color),
              const SizedBox(height: 6),
              Text(label, style: GoogleFonts.spaceGrotesk(fontSize: 10, fontWeight: FontWeight.w800)),
            ],
          ),
        ),
      ),
